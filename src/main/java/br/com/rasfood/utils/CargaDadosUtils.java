...
public class CargaDadosUtils {
    ...
    public static void cadastrarClientes(EntityManager entityManager){
        ...
        enderecoDao.salvar(augusta);
        clienteDao.salvar(felipe);
        enderecoDao.salvar(rioVermelho);
        clienteDao.salvar(cleber);
        enderecoDao.salvar(leblon);
        clienteDao.salvar(calvin);
        enderecoDao.salvar(heitorPenteado);
        clienteDao.salvar(tayane);
        enderecoDao.salvar(consolacao);
        clienteDao.salvar(denise);
        enderecoDao.salvar(nacoesUnidas);
        clienteDao.salvar(claudia);
    ...
    public static void cadastrarOrdensClientes(EntityManager entityManager){
        ...
        OrdemDao ordensDao = new OrdemDao(entityManager);
        List<Cliente> clientes = clienteDao.listar();
        List<Cardapio> cardapioList = cardapio.listar();

        Ordem ordemFelipe = new Ordem(clientes.get(0));
        ordemFelipe.addOrdensCardapio(new OrdensCardapio(cardapioList.get(1),2));
        ordemFelipe.addOrdensCardapio(new OrdensCardapio(cardapioList.get(5),3));

        Ordem ordemCleber = new Ordem(clientes.get(1));
        ordemCleber.addOrdensCardapio(new OrdensCardapio(cardapioList.get(1),1));
        ...
        ordensDao.salvar(ordemFelipe);
        ordensDao.salvar(ordemCleber);
        ordensDao.salvar(ordemCalvin);
        ordensDao.salvar(ordemTayane);
        ordensDao.salvar(ordemDenise);
        ordensDao.salvar(ordemClaudia);
